...
bool IOLoginData::loadPlayer(const std::shared_ptr<Player> &player, const DBResult_ptr &result, bool disableIrrelevantInfo /* = false*/) {
	...
	try {
		...
		// ★ NOVO: carregar presidente/governador/reino
		player->setPresident(result->getNumber<uint32_t>("is_president") != 0);
		player->setGovernor(result->getNumber<uint32_t>("is_governor") != 0);
		player->setKingdom(static_cast<uint8_t>(result->getNumber<uint32_t>("kingdom")));
...
bool IOLoginData::savePlayerGuard(const std::shared_ptr<Player> &player) {
	...
	// ★ NOVO: atualizar presidente/governador/reino no banco
	{
		std::ostringstream q;
		q << "UPDATE `players` SET "
		  << "`is_president`=" << (player->isPresident() ? 1 : 0) << ", "
		  << "`is_governor`=" << (player->isGovernor() ? 1 : 0) << ", "
		  << "`kingdom`=" << static_cast<uint32_t>(player->getKingdom())
		  << " WHERE `id`=" << player->getGUID();

		if (!Database::getInstance().executeQuery(q.str())) {
			throw DatabaseException("[IOLoginData::savePlayerGuard] - Failed to update office/kingdom for player: " + player->getName());
		}
	}
